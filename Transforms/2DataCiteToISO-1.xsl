...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gmx="http://www.isotc211.org/2005/gmx"
    ...
    <mdb:MD_Metadata>
      ...
      <mdb:metadataIdentifier>
        <mcc:MD_Identifier>
          <mcc:code>
            <gco:CharacterString>
              <xsl:value-of select="/resource/identifier"/>
            </gco:CharacterString>
          </mcc:code>
          <mcc:description>
            <gco:CharacterString>
              <xsl:value-of select="/resource/identifier/@identifierType"/>
            </gco:CharacterString>
          </mcc:description>
        </mcc:MD_Identifier>
      </mdb:metadataIdentifier>
      <xsl:for-each select="/resource/language">
        <mdb:defaultLocale>
          <lan:PT_Locale>
      ...
      <xsl:for-each select="/resource/contributors/contributor[
      ...
      <mdb:identificationInfo>
        <mri:MD_DataIdentification>
          ...
          <mri:abstract>
            <xsl:call-template name="writeCharacterString">
              <xsl:with-param name="stringToWrite" select="/resource/descriptions/description"/>
            </xsl:call-template>
          </mri:abstract>
          <xsl:call-template name="writeCodelistElement">
          ...
          -->
          <xsl:for-each select="/resource/contributors/contributor[@contributorType = 'ContactPerson' or @contributorType = 'DataManager']">
            <xsl:call-template name="writeResponsibility">
          ...
          <xsl:for-each select="/resource/formats/format">
            <mri:resourceFormat>
              <mrd:MD_Format>
          ...
          <mri:descriptiveKeywords>
            <mri:MD_Keywords>
              <xsl:for-each select="/resource/subjects/subject">
                <mri:keyword>
                  <gco:CharacterString>
                    <xsl:value-of select="."/>
                  </gco:CharacterString>
          ...
          <!--
            Place Keywords
          -->
          <xsl:for-each select="/resource/geoLocations/geoLocation/geoLocationPlace">
            <mri:descriptiveKeywords>
              <mri:MD_Keywords>
                <xsl:for-each select="/resource/geoLocations/geoLocation/geoLocationPlace">
                  <mri:keyword>
          ...
          <xsl:for-each select="/resource/rightsList/rights">
  ...
  <xsl:template name="writeResourceCitation">
    <cit:CI_Citation>
      ...
      <xsl:for-each select="/resource/titles/title">
        <xsl:choose>
      ...
      <xsl:for-each select="/resource/dates/date">
        <cit:date>
          <cit:CI_Date>
            <cit:date>
              <gco:DateTime>
          ...
          <xsl:choose>
            ...
            <xsl:otherwise>
              <cit:CI_Individual>
                <cit:name>
                  <xsl:call-template name="writeCharacterString">
                    <xsl:with-param name="stringToWrite" select="./contributorName"/>
                  </xsl:call-template>
                </cit:name>
              </cit:CI_Individual>
            </xsl:otherwise>
  ...
  <xsl:template name="writeExtentBoundingBox">
    <xsl:for-each select="/resource/geoLocations/geoLocation/geoLocationPoint">
    ...
    <xsl:for-each select="/resource/geoLocations/geoLocation/geoLocationBox">
