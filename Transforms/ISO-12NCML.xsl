<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0" xmlns:nc="http://www.unidata.ucar.edu/namespaces/netcdf/ncml-2.2" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  xmlns:gco="http://standards.iso.org/iso/19115/-3/gco/1.0" xmlns:gmx="http://www.isotc211.org/2005/gmx" xmlns:gml="http://www.opengis.net/gml/3.2" xmlns:xlink="http://www.w3.org/1999/xlink"
  xmlns:eos="http://earthdata.nasa.gov/schema/eos" exclude-result-prefixes="cat cit gcx gmx gex lan srv mas mcc mco mda mdb mds mdt mex mmi mpc mrc mrd mri mrl mrs msr mdq mac gco gml xlink eos">
      ...
      <xsl:when test="count(@*)">
        ...
        <xsl:element name="nc:group">
          ...
          <xsl:call-template name="writeNcmlAttribute">
            ...
            <xsl:with-param name="value" select="."/>
            <xsl:with-param name="type" select="'gml:attribute'"/>
          </xsl:call-template>
        </xsl:element>
      </xsl:when>
