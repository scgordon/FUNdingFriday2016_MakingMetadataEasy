...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:gco="http://www.isotc211.org/2005/gco" xmlns:gmi="http://www.isotc211.org/2005/gmi" xmlns:gmx="http://www.isotc211.org/2005/gmx"
  ...
  xmlns:mrd="http://standards.iso.org/iso/19115/-3/mrd/1.0" xmlns:mri="http://standards.iso.org/iso/19115/-3/mri/1.0" xmlns:mrs="http://standards.iso.org/iso/19115/-3/mrs/1.0" xmlns:msr="http://standards.iso.org/iso/19115/-3/msr/1.0" xmlns:mdq="http://standards.iso.org/iso/19115/-3/mdq/1.0" xmlns:eos="http://earthdata.nasa.gov/schema/eos" xmlns:dcite="http://datacite.org/schema/kernel-3" xmlns:mods="http://www.loc.gov/mods/v3"
  exclude-result-prefixes="gmd gsr gss gts xs gmi gmx xsi srv1">
  <xd:doc xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" scope="stylesheet">
    <xd:desc>
      <xd:p>
        <xd:b>Title:MODS to ISO 19115-1</xd:b>
      </xd:p>
      <xd:p><xd:b>Version:</xd:b>2017-01-08</xd:p>
      <xd:p><xd:b>Created on:</xd:b>January 8, 2017</xd:p>
      <xd:p><xd:b>Author:</xd:b>thabermann@hdfgroup.org, scgordon@hdfgroup.org</xd:p>
      <xd:p>This stylesheets transforms MODS to ISO 19115-1</xd:p>
      <xd:p/>
    </xd:desc>
  </xd:doc>
  <xsl:variable name="translationName" select="'MODS2ISO-1.xsl'"/>
  <xsl:variable name="translationVersion" select="'2017-03-06'"/>
    ...
    <mdb:MD_Metadata>
      ...
      <mdb:metadataIdentifier>
        <mcc:MD_Identifier>
          ...
          <mcc:description>
            <gco:CharacterString>
              <xsl:value-of select="//mods:mods/mods:identifier/@type"/>
            </gco:CharacterString>
          </mcc:description>
        </mcc:MD_Identifier>
      </mdb:metadataIdentifier>
      <xsl:for-each select="//mods:mods/mods:language">
        <mdb:defaultLocale>
          <lan:PT_Locale>
      ...
      <!-- here --> <xsl:for-each select="/dcite:resource/dcite:contributors/dcite:contributor[
              ...
              <xsl:choose>
                <xsl:when test="//mods:mods/mods:originInfo/mods:dateCreated">
                  <xsl:call-template name="writeDateTime">
                    <xsl:with-param name="dateTimeString" select="//mods:mods/mods:originInfo/mods:dateCreated"/>
                  </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
      ...
      <mdb:identificationInfo>
        <mri:MD_DataIdentification>
          ...
          <mri:abstract>
            <xsl:call-template name="writeCharacterString">
              <xsl:with-param name="stringToWrite" select="//mods:mods/mods:abstract"/>
            </xsl:call-template>
          </mri:abstract>
          <xsl:call-template name="writeCodelistElement">
            <xsl:with-param name="elementName" select="'mri:status'"/>
            <xsl:with-param name="codeListName" select="'mcc:MD_ProgressCode'"/>
            <!-- here -->      <xsl:with-param name="codeListValue" select="/Collection/CollectionState"/>
          </xsl:call-template>
          <!--
          ...
          <!-- here -->   <xsl:for-each select="/*/AssociatedBrowseImageUrls/ProviderBrowseUrl">
            <mri:graphicOverview>
          ...
          <xsl:for-each select="/mods/physicalDescription/internetMediaType | /mods/physicalDescription/form">
            <mri:resourceFormat>
              <mrd:MD_Format>
          ...
          <mri:descriptiveKeywords>
            <mri:MD_Keywords>
              <xsl:for-each select="//mods:mods/mods:subject/mods:topic | //mods:mods/mods:genre">
                <mri:keyword>
                  <gco:CharacterString>
                    <xsl:value-of select="."/>
                  </gco:CharacterString>
          ...
          <!--
            Place Keywords
          -->
          <xsl:for-each select="//mods:mods/mods:subject/mods:geographic/cartographics/coordinates">
            <mri:descriptiveKeywords>
              <mri:MD_Keywords>
                <xsl:for-each select="//mods:mods/mods:subject/mods:geographic/cartographics/coordinates">
                  <mri:keyword>
      ...
      <mdb:distributionInfo>
        <mrd:MD_Distribution>
          <mrd:distributor>
            ...
            <!-- here -->  <xsl:for-each select="/*/Contacts/Contact[contains(Role, 'Archive') or contains(Role, 'DATA CENTER CONTACT') or contains(Role, 'Distributor') or contains(Role, 'User Services') or contains(Role, 'GHRC USER SERVICES') or contains(Role, 'ORNL DAAC User Services')]">
              ...
              <mrd:distributionOrderProcess>
                <mrd:MD_StandardOrderProcess>
                  <mrd:fees>
                    <xsl:call-template name="writeCharacterString">
                      <!-- here --> <xsl:with-param name="stringToWrite" select="/*/Price"/>
                    </xsl:call-template>
                  </mrd:fees>
                </mrd:MD_StandardOrderProcess>
              </mrd:distributionOrderProcess>
              ...
              <mrd:distributorTransferOptions>
                <mrd:MD_DigitalTransferOptions>
                  <!-- here -->   <xsl:if test="/*/DataGranule/SizeMBDataGranule">
                    <mrd:transferSize>
                      <gco:Real>
                        <!-- here -->  <xsl:value-of select="/*/DataGranule/SizeMBDataGranule"/>
                      </gco:Real>
                    </mrd:transferSize>
                  </xsl:if>
                  <!-- here -->  <xsl:for-each select="/*/OnlineAccessURLs/OnlineAccessURL">
                    <mrd:onLine>
                      <cit:CI_OnlineResource>
                        <cit:linkage>
                          <gco:CharacterString>
                            <!-- here --> <xsl:value-of select="encode-for-uri(URL)"/>
                          </gco:CharacterString>
                        </cit:linkage>
                        <cit:applicationProfile>
                          <xsl:call-template name="writeCharacterString">
                            <!-- here -->       <xsl:with-param name="stringToWrite" select="MimeType"/>
                          </xsl:call-template>
                        </cit:applicationProfile>
                        <cit:description>
                          <xsl:call-template name="writeCharacterString">
                            <!-- here -->    <xsl:with-param name="stringToWrite" select="URLDescription"/>
                          </xsl:call-template>
                        </cit:description>
                      </cit:CI_OnlineResource>
                    </mrd:onLine>
                  </xsl:for-each>
                  <!-- here -->  <xsl:for-each select="/*/OnlineResources/OnlineResource">
                    <mrd:onLine>
                      <cit:CI_OnlineResource>
                        <cit:linkage>
                          <gco:CharacterString>
                        ...
                        <cit:description>
                          <xsl:call-template name="writeCharacterString">
                            <!-- here -->     <xsl:with-param name="stringToWrite" select="Description"/>
                          </xsl:call-template>
                        </cit:description>
                        <xsl:call-template name="writeCodelistElement">
  ...
  <xsl:template name="writeResourceCitation">
    <cit:CI_Citation>
      ...
      <xsl:for-each select="//mods:mods/mods:originInfo/mods:dateModified">
        <cit:date>
          <cit:CI_Date>
            <cit:date>
              <gco:DateTime>
                <xsl:call-template name="writeDateTime">
              <!-- here -->    <xsl:with-param name="dateTimeString" select="/*/LastUpdate"/>
                </xsl:call-template>
              </gco:DateTime>
            </cit:date>
            <xsl:call-template name="writeCodelistElement">
  ...
  <xsl:template name="writeResponsibility">
    ...
    <xsl:element name="{$roleName}">
      <cit:CI_Responsibility>
        ...
        <cit:party>
          <xsl:choose>
            <xsl:when test="/mods/name/affiliation">
              <cit:party>
                <cit:CI_Organisation>
                  <cit:name>
                    <gco:CharacterString>
                      <xsl:value-of select="/mods/name/affiliation"/>
                    </gco:CharacterString>
                  </cit:name>
                  <cit:individual>
                    <cit:CI_Individual>
                      <cit:name>
                        <gco:CharacterString>
                          <xsl:value-of select="concat(//mods:mods/mods:name[@type='personal']/mods:namePart[@type='given'], //mods:mods/mods:name[@type='personal']/mods:namePart[@type='family'])"/>
                        </gco:CharacterString>
                      </cit:name>
                    </cit:CI_Individual>
                  </cit:individual>
            ...
            <xsl:otherwise>
              <cit:CI_Individual>
                <cit:name>
                  <xsl:call-template name="writeCharacterString">
                    <xsl:with-param name="stringToWrite" select="concat(//mods:mods/mods:name[@type='personal']/mods:namePart[@type='given'], //mods:mods/mods:name[@type='personal']/mods:namePart[@type='family'])"/>
                  </xsl:call-template>
                </cit:name>
              </cit:CI_Individual>
            </xsl:otherwise>
